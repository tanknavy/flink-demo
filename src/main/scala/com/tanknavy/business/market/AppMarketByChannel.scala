...
import com.tanknavy.business.HotItems.{MyTrigger, UserBehavior, UvCountWithBloom}
import com.tanknavy.business.HotItems.UserViewBoomFilter.getClass
...
//      })
    ...
    val hotPerWindowStream = dataStream //DataStream[SensorReading]
      ...
      .map( data => ((data.channel, data.behavior), 1L) ) //Tuple2[[Tuple2], Long]
      ...
      //.aggregate( new CountAgge(), new WindowResultFunction() ) //可以预聚合，除了reduce类似的增量聚合还有全窗口聚合,两个参数，一个聚合规则，一个输出数据结构
      ...
      .process(new MarketingCountByChannel()) //窗口内排序
...
class SimulatedEventSource() extends RichSourceFunction[MarketingUserBehavior]{
  ...
  //定义渠道集合
  val channelSets: Seq[String] = Seq("google store","facebook","apple store", "wechat", "webo")
  //定义一个随机数发生器
  val rand = new Random()
...
class MarketingCountByChannel() extends ProcessWindowFunction[((String, String),Long), MarketingViewCount,(String,String),TimeWindow]{
  override def process(key: (String, String), context: Context, elements: Iterable[((String, String), Long)], out: Collector[MarketingViewCount]): Unit = {
    //里面可以去重，也可以排序
    val startTs = new Timestamp(context.window.getStart).toString //窗口起始时间
    val endTs = new Timestamp(context.window.getEnd).toString
    val channel = key._1
    val behavior = key._2
    val count = elements.size //这个key这个窗口内的全部数据
